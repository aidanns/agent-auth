...
- `scripts/serve-agent-auth.sh` — run the agent-auth HTTP server (bootstraps `.venv` if missing)
- `scripts/serve-things-bridge.sh` — run the things-bridge HTTP server (bootstraps `.venv` if missing)
