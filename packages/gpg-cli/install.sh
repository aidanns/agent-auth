...
# Install or upgrade ``gpg-cli`` (the devcontainer-side gpg.program
# replacement that forwards git's sign / verify requests to a host
# gpg-bridge) from a published GitHub release into a stdlib-managed
...
# test).
#
# Running this script installs only the ``gpg-cli`` package and its
# workspace dependency ``agent-auth-common``. It does NOT install
# ``agent-auth`` itself or any of the client CLIs — use the
...
TOOL_NAME="gpg-cli"
PACKAGE_SUBDIR="packages/gpg-cli"
...
# from ``packages/gpg-cli/pyproject.toml`` ``[project] dependencies``
