...
# gpg-bridge) from the monorepo into a uv-managed tool environment.
# Optionally uninstall with ``--uninstall``.
#
# Running this script installs only the ``gpg-cli`` package and
...
echo "  gpg-cli — devcontainer gpg.program replacement forwarding to gpg-bridge"
