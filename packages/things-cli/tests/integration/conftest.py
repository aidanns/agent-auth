...
# ``ThingsBridgeStack`` and the corresponding fixtures
# (``things_bridge_stack``, ``things_bridge_stack_factory``) are owned
# by :mod:`tests_support.integration.plugin` — the shared pytest plugin
# loaded via ``addopts = ["-p", "tests_support.integration.plugin"]``
# in the workspace root ``pyproject.toml``. The dataclass is imported
# here for the ``ThingsCliInvoker`` type annotation; the fixtures are
# consumed by argument name in the downstream fixtures below.
from tests_support.integration.plugin import ThingsBridgeStack
...
def things_cli_invoker(
    things_bridge_stack: ThingsBridgeStack,
