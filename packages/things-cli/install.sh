...
# Install or upgrade ``things-cli`` (the read-only command-line client
# for the Things 3 bridge) from the monorepo into a uv-managed tool
...
# workspace package). It does NOT bring up the bridge or the auth
# server — use the per-service installers under
