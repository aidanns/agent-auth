...
# Install or upgrade ``things-cli`` (the read-only command-line
# client for the Things 3 bridge) from a published GitHub release
# into a stdlib-managed Python virtualenv. Optionally uninstall with
# ``--uninstall``. Optionally point at a directory of locally-built
# wheels with ``--local <dir>`` (used by the release-publish CI
# integration test).
#
# Running this script installs only the ``things-cli`` package and
# its workspace dependency ``agent-auth-common``. It does NOT bring
# up the bridge or the auth server — use the per-service installers
# under ``packages/<service>/install.sh`` for those.

set -euo pipefail

GITHUB_REPO="aidanns/agent-auth"
TOOL_NAME="things-cli"
PACKAGE_SUBDIR="packages/things-cli"
...
SERVICE_WHEEL_PREFIX="things_cli-"
...
# from ``packages/things-cli/pyproject.toml`` ``[project] dependencies``
