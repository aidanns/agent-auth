...
# Install or upgrade ``gpg-bridge`` (the HTTP bridge that delegates
# GPG signing to the host gpg binary for agent-auth-protected
# callers) from a published GitHub release into a stdlib-managed
...
# test).
#
# Running this script installs only the ``gpg-bridge`` package and
# its workspace dependency ``agent-auth-common``. It does NOT
# install ``agent-auth`` itself or any of the client CLIs — use the
# per-service installers under ``packages/<service>/install.sh`` for
# those.

set -euo pipefail

GITHUB_REPO="aidanns/agent-auth"
TOOL_NAME="gpg-bridge"
PACKAGE_SUBDIR="packages/gpg-bridge"
...
SERVICE_WHEEL_PREFIX="gpg_bridge-"
...
# from ``packages/gpg-bridge/pyproject.toml`` ``[project] dependencies``
