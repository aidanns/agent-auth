...
This file holds the in-process unit-shard subset (substituted-``gpg``
script-on-disk cases). The end-to-end HTTP variant that drives the
``gpg_backend_fake`` Python module subprocess lives under
``tests/integration/test_wedged_gpg_subprocess.py`` because the unit
shard's bare ``pytest`` invocation does not source
``scripts/_bootstrap_venv.sh``, which is the only place
``packages/gpg-bridge/tests`` is exported on ``PYTHONPATH`` for
subprocess imports of the fake (issue #520).
"""
