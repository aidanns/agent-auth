...
``things_client_fake`` CLI (owned by the things-bridge package). The
fixture here builds the standard test image and exposes a helper that
runs the fake CLI inside a short-lived container, so the wire-protocol
expectations are pinned by the same Docker pattern as the other
services.
...
@dataclass
class FakeCliRunner:
    """Run ``python -m things_client_fake`` inside the test image."""
...
def fake_cli_runner(_test_image_tags, tmp_path_factory) -> FakeCliRunner:
    # ``things_client_fake`` ships in the
