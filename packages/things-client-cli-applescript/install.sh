...
# Install or upgrade ``things-client-cli-applescript`` (the macOS-only
# AppleScript-backed Things 3 CLI that ``things-bridge`` invokes as a
# subprocess) from a published GitHub release into a stdlib-managed
# Python virtualenv. Optionally uninstall with ``--uninstall``.
# Optionally point at a directory of locally-built wheels with
# ``--local <dir>`` (used by the release-publish CI integration
# test).
#
# Running this script installs only the ``things-client-cli-applescript``
# package and its workspace dependency ``agent-auth-common``. The
# CLI's AppleScript backend requires macOS + Automation permissions
# for Things 3; the package installs on Linux too (some contract-test
# scenarios need that), but running ``todos list`` etc. will fail on
# non-macOS.

set -euo pipefail

GITHUB_REPO="aidanns/agent-auth"
TOOL_NAME="things-client-cli-applescript"
PACKAGE_SUBDIR="packages/things-client-cli-applescript"
...
SERVICE_WHEEL_PREFIX="things_client_cli_applescript-"
...
# from ``packages/things-client-cli-applescript/pyproject.toml`` ``[project] dependencies``
