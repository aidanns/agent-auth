...
# agent-auth-protected clients to the Things 3 API) from a published
...
#
# Running this script installs only the ``things-bridge`` package
# and its workspace dependency ``agent-auth-common``. It does NOT
...
SERVICE_WHEEL_PREFIX="things_bridge-"
...
# from ``packages/things-bridge/pyproject.toml`` ``[project] dependencies``
