...
│                      │            │    ▲                                        │
│  gpg-cli ─────HTTPS───────────────▶  gpg-bridge ─subprocess─▶ gpg-backend-cli   │
│                      │            │       │                       │             │
│                      │            │       │                       ▼             │
│                      │            │       │                    host gpg agent   │
│                      │            │       │ HTTP (validate)    + private keys   │
│                      │            │       └──────────────▶ agent-auth           │
└──────────────────────┘            └─────────────────────────────────────────────┘
```
...
- **gpg-bridge** (host) and **gpg-cli** (devcontainer): a second
  ...
  configurable `allowed_signing_keys` allowlist, and delegates to a
  per-request `gpg-backend-cli-host` subprocess that shells out to
  the host `gpg` agent — the only component that touches private
  signing keys. The bridge contains no GPG-protocol logic of its own
  and never returns backend stderr to the HTTP caller. The
  devcontainer-to-host hop carries the same TLS posture as the
  things-cli ↔ things-bridge link
  ([ADR 0025](design/decisions/0025-tls-for-devcontainer-host-traffic.md)).
  Rationale: [ADR 0033](design/decisions/0033-gpg-bridge-cli-split.md).
...
The `gpg-cli` ↔ `gpg-bridge` ↔ `gpg-backend-cli-host` chain
introduces a second devcontainer-to-host hop, this time guarding
private GPG signing keys. The trust boundary is described in
[ADR 0033](design/decisions/0033-gpg-bridge-cli-split.md); the
...
| Threat                                                           | Impact | Likelihood | Rating | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------------------------------- | ------ | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Argv injection into `gpg-backend-cli-host` subprocess            | High   | Low        | Medium | The bridge constructs argv from a fixed set of typed fields (`--local-user <keyid>`, `--armor`, `--keyid-format <fmt>`); operation is dispatched by literal token (`sign` / `verify`), not by interpolating user input. Long-form gpg flags are never forwarded from the HTTP body. Payload bytes traverse stdin, not argv. Targets: **likelihood**. Rationale: [ADR 0033](design/decisions/0033-gpg-bridge-cli-split.md#subprocess-contract-between-gpg-bridge-and-the-backend-cli). |
| Environment-variable tampering of the backend subprocess         | Medium | Low        | Low    | The bridge spawns the backend with the bridge process's own environment minus inbound HTTP headers; HTTP request headers never reach the subprocess env. `GNUPGHOME` and `gpg.conf` are owned by the host operator, not by the request. Targets: **likelihood**.                                                                                                                                                                                                                      |
| Payload tampering on the wire between `gpg-cli` and `gpg-bridge` | High   | Low        | Medium | TLS protects the devcontainer-to-host hop end-to-end ([ADR 0025](design/decisions/0025-tls-for-devcontainer-host-traffic.md)); the audit log records `payload_sha256` of the bytes the bridge actually fed to the backend, which lets the operator detect a mismatch after the fact. Targets: **likelihood** (TLS) and **impact** (post-hoc detection via audit hash). Rationale: [ADR 0033](design/decisions/0033-gpg-bridge-cli-split.md#audit-log).                                |
| Compromised backend CLI returns a forged signature               | Medium | Low        | Low    | A signature produced by a compromised backend will not verify against the host keyring under the requesting user's pubkey, so the threat reduces to denial-of-service for the signing flow rather than silent forgery. Targets: **impact**. Rationale: [ADR 0033 § Security](design/decisions/0033-gpg-bridge-cli-split.md#security).                                                                                                                                                 |
...
| Backend stderr leaked to the HTTP caller                       | Medium | Low        | Low    | Bridge forwards backend stderr only to the bridge's own stderr (line-by-line, capped at a 64 KiB tail for the timeout diagnostic); the HTTP response body never carries stderr text. Targets: **likelihood**. Rationale: [ADR 0033 § Subprocess contract](design/decisions/0033-gpg-bridge-cli-split.md#subprocess-contract-between-gpg-bridge-and-the-backend-cli).                                                                                                                                                |
| Private signing key extracted via the bridge                   | High   | Low        | Medium | The bridge calls only `gpg-backend-cli-host sign` / `verify`. The supported argv set in `gpg-cli` excludes `--export-secret-keys`, `--list-secret-keys`, `--edit-key`, etc.; any other argv shape exits 2 with `unsupported_operation`. The host `gpg-agent` policy and OS file permissions on `~/.gnupg` are the underlying mitigation. Targets: **likelihood**. Rationale: [ADR 0033 § Supported gpg CLI surface in gpg-cli](design/decisions/0033-gpg-bridge-cli-split.md#supported-gpg-cli-surface-in-gpg-cli). |

#### Denial of service — gpg-bridge

| Threat                                            | Impact | Likelihood | Rating | Mitigation                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------- | ------ | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Oversized payload exhausting the bridge / backend | Low    | Medium     | Low    | Request bodies are capped at 1 MiB and rejected with HTTP 413 before the backend is spawned. Commit payloads in practice are a few kilobytes. Targets: **likelihood**. Rationale: [ADR 0033 § HTTP API](design/decisions/0033-gpg-bridge-cli-split.md#http-api-between-gpg-cli-and-gpg-bridge).                                                                                 |
| Hung backend process pinning the bridge           | Medium | Low        | Low    | The bridge enforces `request_timeout_seconds` (default 35 s) on each backend invocation and SIGKILLs after the timeout; the bounded 64 KiB stderr tail keeps memory under control. Targets: **likelihood** + **impact**. Rationale: [ADR 0033 § Subprocess contract](design/decisions/0033-gpg-bridge-cli-split.md#subprocess-contract-between-gpg-bridge-and-the-backend-cli). |
...
| Backend subprocess escalates beyond signing                        | High   | Low        | Medium | `gpg-backend-cli-host` runs as the same user as `gpg-bridge` and has no setuid bit; there is no privilege boundary inside the host to cross. Defence-in-depth comes from the supported-argv surface (sign / verify only) and the JSON envelope contract that fails closed on any unexpected backend output. Targets: **likelihood**. Rationale: [ADR 0033 § Security](design/decisions/0033-gpg-bridge-cli-split.md#security).                                                                                                                                                           |
