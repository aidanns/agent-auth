...
2. **`scripts/lint.sh`** — extend the existing shellcheck-only script
...
9. **Source tree reformatted** — `ruff format` applied across `src/`
...
1. **Add `ruff` to dev extras and configure in `pyproject.toml`** — add
...
2. **Rebuild the venv and run `ruff format` + `ruff check --fix`** —
...
3. **Address residual `ruff check` findings** — anything not auto-fixed
...
4. **`scripts/lint.sh`** — keep the shellcheck flow. Add a parallel
...
5. **`scripts/format.sh`** — mirror `lint.sh`. Add a block that runs
...
7. **`lefthook.yml`** — add two pre-commit commands (`ruff-check` and
...
8. **CI workflow** — `check.yml` already runs `task check`. Because
   ...
   the gate.
9. **`scripts/verify-standards.sh`** — extend the bash gating block with
