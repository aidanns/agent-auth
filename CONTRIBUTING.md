...
| `Closes: #N` / `Fixes: #N` / `Resolves: #N` | Link a GitHub issue. The merge bot pastes the block as the squash-merge body, then closes the linked issue itself via the issues API. All three spellings work; the colon form is required so it matches every other trailer (the bare `Closes #N` was dropped by the validator in #566 — see `pr_lint_validator.commit_msg`). |
