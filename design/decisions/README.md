...
- [ADR 0038 — Merge bot via dedicated GitHub App + `automerge` label](0038-merge-bot-via-github-app.md)
  — `.github/workflows/merge-bot.yml` extracts the PR's `==COMMIT_MSG==` block via `scripts/extract-commit-msg-block.py` and calls `PUT /pulls/{n}/merge` with `merge_method: squash`, replacing the maintainer-paste step from ADR 0037; triggered by the `automerge` label, refuses to merge on lint or required-check failure, requires a `Signed-off-by:` trailer in the block. Maintainer setup of the dedicated `agent-auth-merge-bot` App is in `docs/release/merge-bot-setup.md`.
- [ADR 0039 — Bot-mediated changelog authoring](0039-bot-mediated-changelog-authoring.md)
  — `agent-auth-changelog-bot` GitHub App reads `==CHANGELOG_MSG==` and `==NO_CHANGELOG==` markers from PR descriptions and either commits a `changelog/@unreleased/pr-<N>-*.yml` file or applies the `no changelog` label. Reconciliation with manual edits is via author-history lockout; `pull_request: synchronize` re-triggers from the bot's own push are filtered by a workflow-level `if:` plus a head-commit-author check.
- [ADR 0040 — Per-service Docker integration tests for the gpg-bridge surface](0040-gpg-bridge-docker-integration-tests.md)
  — extends the ADR 0005 / ADR 0034 pattern to the gpg signing workflow with two new `<svc>-test` images, profile-gated compose services, and a baked throwaway `GNUPGHOME`; replaces the `_NoopAuthz`-stubbed `test_gpg_end_to_end.py` smoke test with an `integration-gpg-bridge` job that gates real `gpg --verify` and the live agent-auth authz path on every PR. Closes #278.
- [ADR 0041 — YAML-driven release workflow with a maintainer-merged release PR](0041-yaml-driven-release-workflow.md)
  — replaces semantic-release with two GitHub Actions: `release-pr.yml` opens / refreshes a `release/X.Y.Z` PR on every push to `main` from the `changelog/@unreleased/*.yml` files, and `release-tag.yml` tags + publishes when that PR merges. The artefact pipeline (`release-publish.yml`, SLSA-L3, cosign, SBOMs) is unchanged. Decommissions `.releaserc.mjs`, `package.json`, and the npm Dependabot ecosystem; supersedes ADR 0026 on the autorelease driver.
- [ADR 0042 — Bridge-owned signing-key passphrase store in the system keyring](0042-gpg-bridge-passphrase-store.md)
  — `gpg-bridge` gains a per-fingerprint `KeyringPassphraseStore` and a `gpg-bridge passphrase set / clear / list` CLI surface; when a passphrase is stored for the requested signing key, the host `gpg` subprocess receives it via `--passphrase-fd`, removing the dependency on `gpg-agent`'s cache. Builds on ADR 0008 (system keyring for key material) and ADR 0033 (gpg-bridge / gpg-cli split); the bridge process now holds unlocked passphrase material in memory for the duration of one sign. Opt-out via `passphrase_store_enabled: false`.
- [ADR 0043 — Share the refresh + reissue retry loop across CLIs](0043-shared-authenticated-retry-library.md)
  — `AuthenticatedRetry` in `agent_auth_client` owns the single-retry refresh + reissue ladder; `things-cli` and `gpg-cli` each construct one with their own typed exception classes and credential record. Sibling to ADR 0030 (per-service HTTP clients) — same drift-by-duplication argument applied one layer up. Closes #328.
- [ADR 0044 — Per-package wheels as release assets, workspace deps shipped alongside](0044-per-package-release-asset-layout.md)
  — `release-publish.yml` builds via `uv build --all-packages` and uploads one wheel + sdist + `.sha256` + SBOM + cosign bundle per workspace package; SLSA provenance enumerates every wheel + sdist subject. `packages/<svc>/install.sh` no longer requires `uv` — Python 3.11 + stdlib `venv` is enough — and downloads the service wheel + every workspace-dep wheel into `~/.local/share/<svc>/venv` with sha256 verification. `--local <dir>` lets a CI integration test (`install-from-wheels` in `test.yml`) exercise the install path end-to-end without a published release. Closes #324.
- [ADR 0045 — `Co-Authored-By: Claude` trailer in `==COMMIT_MSG==` blocks](0045-claude-coauthorship-trailer.md)
  — every Claude-authored squash-merge body carries a `Co-Authored-By: Claude <model+context> <noreply@anthropic.com>` trailer between `Closes:` and `Signed-off-by:` so AI co-authorship survives the squash into `git log` and the GitHub UI. Author-responsibility rule, enforced via PR template comment + CONTRIBUTING.md "Writing PRs" → "Claude attribution trailer" + a CLAUDE.md index entry. Bot-authored PRs (release-bot, changelog-bot, dependabot adaptor) are exempted by natural fall-out — their blocks are rendered from YAML with no Claude in the loop. No retroactive backfill; no validator change in v1. Builds on ADR 0037 and ADR 0038. Closes #553.
- [ADR 0046 — Flat `required-checks-passed` aggregator and kebab-case workflow / action names](0046-flat-required-checks-aggregator-and-kebab-case-workflow-names.md)
  — workflow `name:` = filename minus `.yml`; composite-action `name:` = directory name; non-matrix job `name:` field is dropped (job ID becomes the rendered check-run name); one repo-wide `required-checks-passed` aggregator at the top of `ci.yml` (no intermediate aggregators in `check-fmt.yml` / `check-lint.yml` / `check-security.yml` / `check-pull-request.yml` / `nightly.yml` / `weekly.yml`). `scripts/ci/diff-dependent-jobs.sh` reworked to enumerate leaves dynamically from the workflow YAML. Restores #440's locked-in single-aggregator decision and partially supersedes the naming half of #414. Closes #556.
- [ADR 0047 — Vulture as a per-package soft dead-code gate](0047-vulture-soft-dead-code-gate.md)
  — `scripts/check-dead-code.sh` runs vulture per workspace package at `--min-confidence 80` from a new `dead-code` job in `check-lint.yml`; findings land in the workflow summary but the job always exits `success` (advisory). Suppression is annotation-driven via `# noqa: F841,RUF100` at the call site (no whitelist module). Promotion from soft to required is tracked in #593. Closes #578.
- [ADR 0048 — Per-package dependency license allowlist gate](0048-dependency-license-allowlist-gate.md)
  — `scripts/check-license-allowlist.sh` runs per-package via the `check-license-allowlist.yml` matrix child of `ci.yml`. Allowlist is `MIT / Apache-2.0 / BSD-2-Clause / BSD-3-Clause / ISC / Python-2.0 / MPL-2.0`; rejects `GPL-*` / `LGPL-*` / `AGPL-*` / `SSPL-*` / `BUSL-*`. SPDX `A OR B` picks the first allowlisted alternative; `A AND B` requires every component on the allowlist. Dev and runtime get the same gate. Per-package YAML exceptions under `packages/<svc>/licenses.exceptions.yml` with mandatory `reason` + `expires`. Closes #575.
- [ADR 0049 — macOS Tart-VM CI lane with sqlite3-direct TCC seed](0049-macos-tart-ci-tcc-seed.md)
  — `.github/workflows/test-system-macos-tart.yml` boots a `cirruslabs/macos-sequoia-base` guest under Apple's Virtualization.framework via the SHA-pinned Tart CLI, brew-installs Things 3, seeds an `kTCCServiceAppleEvents` Automation grant for `osascript` -> `com.culturedcode.ThingsMac` directly into TCC.db (SIP off in the base image), and exercises a `things-client-cli-applescript todos list --status open` round-trip against a real Things 3 instance. Lands as standalone plumbing in this PR; the `ci.yml` wiring + smoke test land in a follow-up PR (split per #485 chicken-and-egg: GitHub Actions only registers reusable workflows from `main`). Pre-baked image and full `agent-auth -> things-bridge -> CLI -> Things 3` e2e are deferred (#568, #567). Closes #485.
