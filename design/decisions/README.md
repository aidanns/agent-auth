...
- [ADR 0047 — macOS Tart-VM CI lane with sqlite3-direct TCC seed](0047-macos-tart-ci-tcc-seed.md)
  — `.github/workflows/test-system-macos-tart.yml` boots a clean macOS guest under Apple's Virtualization.framework via the Tart CLI on `macos-15`, installs Things 3, seeds an `kTCCServiceAppleEvents` Automation grant for `osascript -> com.culturedcode.ThingsMac` directly into TCC.db (SIP off in the cirruslabs `macos-sequoia-base` image), and runs a smoke test that round-trips a seeded to-do through `things-client-cli-applescript`. Path-filter gating (`things_client_cli_applescript_updated`, `agent_auth_common_updated`, `macos_tart_plumbing_updated`) keeps the `macos-15` runner cost off PRs that don't touch the watched paths. Plumbing only — the full agent-auth -> things-bridge -> CLI -> Things 3 e2e is tracked separately as #567; a pre-baked Tart image (Option A) is deferred to #568 with reactivation criteria. Consequence: the TCC.db schema is now an owned dependency scoped to Sequoia minor versions; OS bumps require seed-script updates. Operator guide at `docs/operations/macos-tart-ci.md`. Closes #485.
