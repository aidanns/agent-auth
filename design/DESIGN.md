...
- **`things-client-cli-fake`** — test-only, never shipped. Reads an in-memory store from a YAML fixture file and lives under `tests/things_client_fake/`. Invoked as `python -m things_client_fake --fixtures PATH`. Lets the agent-auth + things-bridge + things-cli stack run end-to-end on Linux without `osascript` or Things 3.
