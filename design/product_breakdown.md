...
| agent-auth | things-bridge | Component | HTTP server running on the host that proxies read operations to the Things 3 application via AppleScript, delegating authorization to agent-auth. |  |
| things-bridge | things-bridge | Configuration Item | HTTP server service that maps /things-bridge/ endpoints to Things 3 AppleScript interactions, delegating token validation to agent-auth. | Delegate Token Validation, Execute External System Interaction, Serve Bridge HTTP API, Serve Fake Things Client |
