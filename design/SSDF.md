...
| PO.4.2 | Implement processes, mechanisms, etc. to gather and analyze info  | Partial        | CI surfaces failures on PRs and via the daily `pip-audit` schedule. `pip-audit` emits SARIF on every run and `.github/workflows/security.yml` uploads it to the GitHub Security tab (Code scanning alerts); scan-failure notifications still tracked in [#86](https://github.com/aidanns/agent-auth/issues/86).                         |
