...
| HTTP API | Serve Validate Endpoint | Handle POST /agent-auth/validate requests from app bridges, including blocking for JIT approval on prompt-tier scopes. |
| HTTP API | Serve Refresh Endpoint | Handle POST /agent-auth/token/refresh requests from CLIs. |
| HTTP API | Serve Reissue Endpoint | Handle POST /agent-auth/token/reissue requests from CLIs when the refresh token has expired. Triggers JIT approval via the configured notification plugin. |
| HTTP API | Serve Status Endpoint | Handle GET /agent-auth/token/status requests for token introspection. |
